...
[~, Vtf_x, Vtf_xx] = term_costfn(controller.states(end, :)');
Vt_next_x = Vtf_x;
Vt_next_xx = Vtf_xx;
for t = horizon:-1:1
    [cost, cx, cu, cxx, cxu, cuu] = costfn(controller.states(t, :)', controller.controls(t,:)');
    ...
    Qt_uu = cuu + fu'*Vt_next_xx*fu + tensor_product(Vt_next_x, fuu) + dynamic_regularizer*(fu'*fu);
