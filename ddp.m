function [controller, total_costs] = ddp(ic, initial_controls, iters, regularizer, dyn, costfn, term_costfn)
...
%% Your code below
[xs, us, J_0] = fwd_pass(ic, controller, dyn, costfn, term_costfn);

e_M = [1, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1, 0.05, 0.01, 0.005, 0.001, 0.0005, 0.0001];
J_i = J_0;
for i = 1:iters

    controller = back_pass(xs, us, dyn, costfn, term_costfn, regularizer);
    for j = 1:length(e_M)
        controller_j = controller;
        controller_j.k = e_M(j)*controller.k;
        [xs_j, us_j, J_j] = fwd_pass(ic, controller_j, dyn, costfn, term_costfn);
        if sum(J_j) < sum(J_i)
            xs = xs_j;
            us = us_j;
            J_i = J_j;
